...
(test-case
  ...
  (check-equal? (deriv '((10 + 3 * x) + 3 * (x + y + 2)) 'x) 6
                "combination does not work correctly.")
  (check-equal? (deriv '(10 + 3 * x + (5 + 4 * y * x)) 'x)
                '(3 + (4 * y))
                "combination does not work correctly.")
  (check-equal? (deriv  '(10 * x ** 2 * 4) 'x)
                '(10 * ((2 * x) * 4))
                "combination does not work correctly.")
  (check-equal? (deriv  '(2 * x ** 3 * 4) 'x)
                '(2 * ((3 * (x ** 2)) * 4))
                "combination does not work correctly."))
...
(test-case "take-until test case"
           (check-equal? (take-until '(1 2 3 a b) 'a)
                         '(1 2 3)
                         "The procedure take-until does not work correctly.")
           (check-equal? (take-until '(1 2 3 a b) 'c)
                         '(1 2 3 a b)
                         "The procedure take-until does not work correctly."))
