...
(define (successive-merge ordered-pairs-set)
  (define (recurse ordered-pairs-set)
    (cond
      [(empty? ordered-pairs-set) nil]
      [(= (length ordered-pairs-set) 1)
       (let
         ([remaining-pair (car ordered-pairs-set)])
         (make-leaf (car remaining-pair) (cdr remaining-pair)))]
      [else
        (let*
          ([current-pair (car ordered-pairs-set)]
           [a-symbol (car current-pair)]
           [a-weight (cdr current-pair)])
          (combine-subtrees
            (make-leaf a-symbol a-weight)
            (recurse (cdr ordered-pairs-set))))]))
  ;; If we define it like this, we only need to reverse the list once, instead of in the procedure.
  (recurse (reverse ordered-pairs-set)))
